...
\section{在 Linux (openSUSE 系发行版) 下安装 Visual Studio Code}
\label{Installation_VSCode_openSUSE}

在 openSUSE 系发行版下，可以通过如下方式将 VSCode 加入软件源，从而便于安装和更新。
...
使用这个命令来更新软件源缓存并安装 VSCode：
\begin{lstlisting}[language=bash]
    sudo zypper install code # 如果你喜欢尝鲜，把 code 换成 code-insiders
\end{lstlisting}

当然，你也可以选择使用 YaST 软件管理或者 Myrlyn 软件管理工具来完成最后的安装步骤。
